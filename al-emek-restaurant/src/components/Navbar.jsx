...
export default function Navbar({ activePage, setActivePage }) {
  ...
  const navItems = [
    ...
    { id: 'deliveries', label: t('nav.deliveries') || (lang === 'ar' ? 'التوصيل' : lang === 'he' ? 'משلوחים' : 'Deliveries') },
    { id: 'contact', label: t('nav.contact') || (lang === 'ar' ? 'اتصل بنا' : lang === 'he' ? 'צור קשר' : 'Contact') },
  ];

  // Language configuration with native titles
  const languages = [
    { code: 'he', label: 'עברית' },
    { code: 'ar', label: 'عربية' },
  ...
  return (
    ...
    >
      ...
      {/* 1. TOP BAR: Logo + Large Social Links + Prominent Language Switcher */}
      <div className="max-w-7xl mx-auto px-2 sm:px-6 py-2.5 flex items-center justify-between border-b border-white/5 gap-1.5 sm:gap-4">
        ...
        >
          ...
          <span className="text-xs sm:text-2xl font-black bg-gradient-to-r from-[#F3E5AB] via-[#D4AF37] to-[#AA771C] bg-clip-text text-transparent tracking-wide drop-shadow-sm">
            {lang === 'ar' ? 'مطعم العيمق' : lang === 'en' ? 'Al-Emek' : 'מסעדת העמק'}
          </span>
        </div>

        {/* Right Section: Bigger Social Icons + Bigger Language Switcher */}
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          
          {/* Social Media Links (Bigger Buttons) */}
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            ...
            <a
              ...
              aria-label="TikTok"
              className="w-8 h-8 sm:w-11 sm:h-11 bg-[#181818] hover:bg-[#00F2FE] hover:text-black text-gray-300 rounded-full border border-[#D4AF37]/40 flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-md"
            >
            ...
            <a
              ...
              className="w-8 h-8 sm:w-11 sm:h-11 bg-[#181818] hover:bg-gradient-to-tr hover:from-amber-500 hover:via-rose-500 hover:to-purple-600 hover:text-white text-gray-300 rounded-full border border-[#D4AF37]/40 flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-md"
            >
              <svg className="w-4 h-4 sm:w-5 sm:h-5 fill-current" viewBox="0 0 24 24">
            ...
            <a
              ...
              rel="noopener noreferrer"
              aria-label="Facebook"
              className="w-8 h-8 sm:w-11 sm:h-11 bg-[#181818] hover:bg-[#1877F2] hover:text-white text-gray-300 rounded-full border border-[#D4AF37]/40 flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-md"
            >
              <svg className="w-4 h-4 sm:w-5 sm:h-5 fill-current" viewBox="0 0 24 24">
          ...
          {/* Larger Language Switcher Pill */}
          <div className="flex bg-[#161616] p-1 sm:p-1.5 rounded-2xl border border-[#D4AF37]/40 shadow-inner gap-1" dir="ltr">
            {languages.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 sm:px-4 py-1 sm:py-1.5 text-xs sm:text-sm font-extrabold rounded-xl transition-all duration-300 whitespace-nowrap active:scale-95 ${
                    isActive 
